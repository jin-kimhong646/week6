...
function ArticleCard({ article, onView, onEdit, onDelete }) {
  // Make sure we have a valid journalist_id
  const journalistId = article.journalist_id || article.journalistid;

  if (!journalistId) {
    console.warn("No journalist ID found for article:", article);
  }
  ...
  return (
    <div className="article-card">
      ...
      <div className="article-author">
        {journalistId ? (
          <Link
            to={`/journalists/${journalistId}/articles`}
            className="journalist-link"
          >
            {article.journalist_name || "Unknown Journalist"}
          </Link>
        ) : (
          <span>{article.journalist_name || "Unknown Journalist"}</span>
        )}
      </div>
